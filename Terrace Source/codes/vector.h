...
public:
  ...
  inline Vectors(std::istream &is) {
    is.read((char *) &size, sizeof(size));
    content = new T[size];
    is.read((char *) content, sizeof(T)*size);
  }

  inline Vectors() : size(0), content(0) { }
  inline Vectors(int s) : size(s), content(new T[size]) {}
  inline Vectors(const Vectors &v) : size(v.size), content(new T[size]) {
...
//   }
  ...
  inline Vectors &operator = (const Vectors&v) {
    if(this != &v) {
      if(v.size != size) {
	delete[] content;
	size = v.size;
  ...
  inline bool operator == (const Vectors&v) {
    if(this != &v) {
...
template<class T>
std::ostream &operator << (std::ostream &os, const Vectors<T> &v) { v.print(os); return os; }
